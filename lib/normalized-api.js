...
async function getLocalDevUser(sql) {
  const appUsersExists = await hasTable(sql, "app_users");
  if (!appUsersExists) return null;
  ...
  if (existingRows[0]) {
    return mapNormalizedUser(existingRows[0]);
  }

  const company = await getSingleCompany(sql);
  ...
  return mapNormalizedUser(rows[0]);
}
...
export async function getRequestContext() {
  if (isLocalDevAuthBypassEnabled()) {
    ...
    return {
      ...
      appUser: await getLocalDevUser(sql),
      sql,
    };
  }
