  ...
  useEffect(() => {
    ...
    async function loadJob() {
      try {
        ...
        // Check if M1 or M2 already exists for this job
        ...
        if (invoicesRes.ok) {
          ...
          if (existing) {
            setError(`${type} invoice already exists for this job`);
          }
        }
      } catch (err) {
  ...
  return (
    <AppShell>
      ...
      <div className="page-header">
        ...
        <p>
          {job ? `${job.customerName} • Job ${job.jobNumber}` : "Loading job details..."}
        </p>
      </div>
        ...
        <div className="card card-elevated">
          <form onSubmit={handleSubmit} style={{ display: "grid", gap: 14 }}>
            <FormField label="Invoice type">
              ...
              {typeParam && (
                <span style={{ fontSize: 11, color: "var(--text-secondary)" }}>Type specified in request</span>
              )}
            </FormField>

            <FormField label="Invoice number" span={1}>
              <input
            ...
            <FormField label="Description">
              <input
                ...
                placeholder="e.g., Milestone 1 - Equipment and labor"
              />
            </FormField>
